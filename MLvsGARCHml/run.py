...
def run(config,
        classification=True,
        training=True):
    data_param, training_param, cv_param, layers = config["data_param"], config["training_param"],\
                                                   config["cv_param"], config["model"]

    label = config["label"]
    if 'label_param' in config.keys():
        label_param = config['label_param']

    if 'class_weight' in training_param.keys():
        class_weight = training_param['class_weight']
    else:
        class_weight = False

    if data_param['type'] == 'load':
        lookfront = label_param['lookfront']
        n_classes=data_param['n_classes']
        normalization=data_param['normalization']
        metrics = ['accuracy']

    elif data_param['type'] == 'sinc_simulation':
        lookfront = 0
        n_classes=0
        normalization=None
        metrics = ['mse']
    ...
    if data_param['type'] == 'load':
        ...
        dfdata, target, feature_names = sync_simulation(**data_param['params'])

    print(dfdata.head())

    if config['label'] == 'labelQuantile':
        dfdata.to_pickle('%s/dfdata.p' % model_dir)
    elif config['label'] == 'sinc':
        dfdata.to_pickle('%s/dfdata.p' % model_dir)
    ...
    if training:
        for cv_split_i in range(cv_param['cv_split']):
            ...
            plot_history(model, model.model_dir, 'e={}'.format(training_param['n_epochs']))
        ...
        for cv_split_i in range(cv_param['cv_split']):
            ...
            model = Model(model_dir='{}/{}/model_{}.h5'.format(model_dir, cv_split_i, epoch_number), metrics = metrics)
            model.load_model()
